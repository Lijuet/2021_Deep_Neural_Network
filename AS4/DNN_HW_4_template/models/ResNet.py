...
class ResNet(nn.Module):
    def __init__(self, input_channel, output_dim, learning_rate, reg_lambda, device):
        ...
        self.CONV2 = self._make_layer(BasicBlock, 64, 2, 1)
        self.CONV3 = self._make_layer(BasicBlock, 128, 2, 2)
        self.CONV4 = self._make_layer(BasicBlock, 256, 2, 2)
        self.CONV5 = self._make_layer(BasicBlock, 512, 2, 2)

        self.POOL2 = nn.AvgPool2d(kernel_size=(1,1))

        self.FC = nn.Linear(7*7*512, output_dim)

        self.Conv_layers = nn.Sequential(self.CONV1, self.BNORM, nn.ReLU(), self.POOL1, self.CONV2, self.CONV3, self.CONV4, self.CONV5, self.POOL2)

        self.loss_function = nn.CrossEntropyLoss()
        self.optimizer = torch.optim.SGD(self.parameters(), lr=learning_rate, weight_decay=reg_lambda)
